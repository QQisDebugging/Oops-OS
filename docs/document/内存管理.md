...
- **置换策略**：`swapout` 采用近似时钟算法（Clock）。以 `hand_va` 为扫描指针，先从 `hand_va` 到 `p->sz` 清除 A 位，再从 0 到 `hand_va` 清除 A 位，必要时第三轮忽略 A 位强制淘汰，避免“全是热页”时无法换出。仅选择用户页（`PTE_U`）且引用计数为 1 的页面；对 `MAP_SHARED` 文件映射页跳过，以保持共享写回语义。
