...
当物理内存不足时，系统将部分用户页写入磁盘的 swap 区域，释放出物理页；之后访问到这些页面会触发缺页异常，再将页面从 swap 区域读回内存并恢复映射。

#### 2.5.2 Swap 区域与元数据

- 使用固定大小的 swap 区域，位于文件系统元数据之后的连续磁盘块中。
- 每个 swap 槽对应 1 个用户页（4 个磁盘块）。
- 采用位图管理 swap 槽分配与回收。

#### 2.5.3 置换/换入流程

- **换出**：当 `kalloc` 无空闲页时触发 `swapout`，在进程页表中选择可置换页面写入 swap 槽，清除 `PTE_V` 并设置 `PTE_S`，同时在 PTE 中记录槽号。
- **换入**：访问到被换出的页触发缺页异常，`swapin` 读取 swap 槽数据并恢复页表映射。
- **fork 处理**：`uvmcopy` 遇到 `PTE_S` 时复制 swap 槽数据，为子进程建立独立的 swap 槽。
- **释放**：`uvmunmap` 在回收映射时释放对应的 swap 槽。

#### 2.5.4 限制与注意点

- 当前仅对普通用户页进行置换，`mmap` 文件映射区域暂不参与换出。
- 置换策略为简单的顺序扫描（近似 FIFO），未使用访问位（A bit）。
- swap 区域大小固定，需要重新生成 `fs.img` 以预留 swap 空间。
