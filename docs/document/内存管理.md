...
OopsOS是我们在xv6基础上改进的内核。文档分为三部分：OopsOS内存管理总览、xv6的基本功能与我们改进与新增的功能。

**目录：**

- [1. 总览](#1-总览)
  - [1.1 内存管理总览](#11-内存管理总览)
...
- [2. 基本功能](#2-基本功能)
  ...
  - [2.3 内核页表](#23-内核页表)
  - [2.4 进程页表](#24-进程页表)
  - [2.5 内存管理主要函数](#25-内存管理主要函数)
- [3. 改进与创新](#3-改进与创新)
  - [3.1 写时复制(Copy On Write)](#31-写时复制copy-on-write)
  - [3.2 懒分配](#32-懒分配)
  - [3.3 基于VMA的文件内存映射(MMAP)](#33-基于vma的文件内存映射mmap)
  - [3.4 细粒度化空闲页链表互斥锁](#34-细粒度化空闲页链表互斥锁)
  - [3.5 页面置换/换入（Swap）](#35-页面置换换入swap)
  - [3.6 动态装入（按需加载）](#36-动态装入按需加载)
  - [3.7 结构化对比与测试覆盖](#37-结构化对比与测试覆盖)
...
图 1-1 展示了 OopsOS 内存管理的核心路径：进程地址空间、VMA、页表与缺页处理之间的关系。

<img src="../img/mm_overview.png" alt="图 1-1 内存管理总览" width="680" />
...
| 改进类别 | 功能点 | 关键实现 | 测试程序 |
|---------|--------|---------|----------|
| **写时复制** | COW fork | 共享物理页、引用计数、写时复制 | `cowtest` |
| **懒分配** | 延迟物理页分配 | sbrk 仅记录大小、缺页时真正分配 | `lazytest` |
| **内存映射** | 文件 mmap/munmap | VMA 管理、MAP_SHARED/PRIVATE、缺页加载 | `mmaptest` |
| **细粒度锁** | 空闲页链表分桶锁 | 多空闲链表、减少锁竞争 | `kalloctest` |
| **页面置换** | Swap 换出/换入 | Clock 算法、交换分区、按需换入 | `swaptest` |
| **动态装入** | 按需加载代码段 | ELF 延迟加载、缺页触发 | `loadtest` |

### 1.3 关键流程图

图 1-2 给出了缺页处理的高层流程，覆盖懒分配、swap 换入与 mmap 缺页路径。

<img src="../img/page_fault_flow.png" alt="图 1-2 缺页处理流程" width="400" />

_图 1-2 缺页处理流程_

图 1-3 展示 swap 置换与换入流程的主干路径。

<img src="../img/swap_flow.png" alt="图 1-3 Swap 置换/换入流程" width="400" />

_图 1-3 Swap 置换/换入流程_

图 1-4 展示 COW 写时复制的触发与复制流程。

<img src="../img/cow_flow.png" alt="图 1-4 COW 写时复制流程" width="230" />

_图 1-4 COW 写时复制流程_

图 1-5 展示文件映射页在缺页与淘汰时的处理路径（MAP_SHARED 写回）。

<img src="../img/mmap_flow.png" alt="图 1-5 mmap 缺页与写回流程" width="350" />

_图 1-5 mmap 缺页与写回流程_

图 1-6 展示标准 Clock（二次机会）算法的扫描与指针位置示意。

<img src="../img/mm_clock_state.png" alt="图 1-6 Clock（二次机会）状态示意" width="400" />

_图 1-6 Clock（二次机会）状态示意_
...
<img src="../img/lazy_alloc_principle.png" alt="图 3-2 懒分配原理示意" width="200" />
...
<img src="../img/lazytest.png" alt="lazytest" style="zoom: 60%;" />
...
<img src="../img/swap_principle.png" alt="图 3-5 Swap 原理示意" width="140" />

#### 3.5.2 实现策略

- **PTE 标记与槽号编码**：使用 `PTE_S` 标识“已换出”状态，槽号编码进 PTE 的物理页号字段。换出时清除 `PTE_V`，换入时恢复 `PTE_V` 并清除 `PTE_S`。
...
- **Swap 区域与槽管理**：根据超级块计算 swap 起始块，连续预留 `SWAPBLOCKS`。位图 `swapmap` 记录槽是否占用，`swap_hint` 作为下次扫描起点，降低多进程竞争时的遍历成本。
...
- **时钟（Second-Chance）置换**：每个进程维护 `swap_hand` 指针，从当前位置开始扫描用户页。遇到 `PTE_A` 先清除并跳过；若 A 位仍为 0，则候选淘汰。扫描至多两轮，避免“热页”被误淘汰。

```c
// kernel/mm/swap.c (核心逻辑)
if (pte && (*pte & PTE_V) && (*pte & PTE_U)) {
  if (*pte & PTE_A) {
    *pte &= ~PTE_A;
    ...
    if (krefcnt((void *)pa) == 1) {
      ...
      uint flags = (PTE_FLAGS(*pte) & ~PTE_V) | PTE_S;
      *pte = SWAP2PTE(slot) | flags;
      sfence_vma();
      swap_pbuf_put(slot, pa);
      p->swap_hand = (va + PGSIZE >= sz) ? 0 : va + PGSIZE;
      return 0;
    }
  }
}
  ...
  - **抖动预防（轻量节流）**：统计短时间窗口内的 swap 事件数，超过阈值时短暂让出 CPU（sleep 数个 tick），降低频繁换入/换出导致的抖动。
  ...
  - **轻量页面缓冲（PBA lite）**：将刚换出的页暂存在一个短链表（swap buffer），若很快再次访问对应 swap 槽，直接复用内存页，避免重复磁盘读取；缓冲满时按 FIFO 淘汰并释放物理页。

  ```c
  // kernel/mm/swap.c
  static void *swap_pbuf_get(uint slot)
  {
    void *pa = 0;
...
- **文件映射页换出（MAP_SHARED）**：当待淘汰页属于 `MAP_SHARED` 的 VMA 时，不写入 swap 槽，而是按页偏移写回文件、清空 PTE 并释放物理页。后续访问由 `mmap_handler` 重新按需加载。
...
- **换入路径**：缺页时优先 `swapin`，分配新页、读取 swap 槽并恢复 PTE。

```c
// kernel/mm/swap.c
int swapin(pagetable_t pagetable, uint64 va)
{
  pte_t *pte = walk(pagetable, va, 0);
  ...
  swapfree(slot);
  uint flags = (PTE_FLAGS(*pte) & ~PTE_S) | PTE_V;
...
- **回收与 fork 复制**：`uvmunmap` 在回收 swapped 页时释放槽位；`uvmcopy` 遇到 swapped 页调用 `swapcopy`，为子进程拷贝 swap 槽内容。

#### 3.5.3 测试程序

`/user/test/swaptest` 是核心压力测试，直接触发 swapout/swapin 并校验模式数据：

```c
// user/test/swaptest.c (核心触发逻辑)
...
`/user/test/mmapswaptest` 验证 `MAP_SHARED` 文件映射页参与置换，数据回写正确：

```c
// user/test/mmapswaptest.c (映射 + 校验)
...
`/user/test/swapconctest` 多进程并发触发 swap，展示并发效率与正确性（带超时保护）：

```c
  // user/test/swapconctest.c (并发压力)
  ...
  `/user/test/swapthrashtest` 多进程反复读写大工作集，制造高频换入/换出场景，验证抖动节流机制不会导致系统崩溃（因为该测试耗时较长，所以默认不纳入 `usertests`，避免全量回归过慢，但可以单独测试）。下面单独附上我们单独测试的结果截图。

   <img src="../img/swapthrashtest.png" alt="swapthrashtest 结果" width="400" />

  `/user/test/swapbuftest` 在轻度超额内存压力下触发 swapout/swapin，并检查 swap buffer 命中统计不为 0。
  `/user/test/swapbufcmptest` 对照测试：顺序读/逆序读对比 swap buffer 命中与耗时（因为该测试耗时较长，所以默认不纳入 `usertests`，避免全量回归过慢，但可以单独测试）下面单独附上我们单独测试的结果截图。

  <img src="../img/swapbufcmptest.png" alt="swapbufcmptest 结果" width="400" />

  `/user/test/swapperftest` 用于性能回归（因为该测试耗时较长，所以默认不纳入 `usertests`，避免全量回归过慢，但可以单独测试），统计多轮全量读写的耗时。下面单独附上我们单独测试的结果截图。

  <img src="../img/swapperftest.png" alt="swapperftest 结果" width="400" />


**测试结果：**

<div align="left">
  <img src="../img/swaptest.png" alt="swaptest 结果" width="360" />
  <br/>
  <img src="../img/mmapswaptest.png" alt="mmapswaptest 结果" width="360" />
  <br/>
  <img src="../img/swapconctest多打印版本（多进程）.png" alt="swapconctest 结果" width="360" />
</div>
...
<img src="../img/demand_load_principle.png" alt="图 3-6 按需装入原理示意" width="120" />

#### 3.6.2 实现策略

- **exec 阶段只建 VMA，不直接 loadseg**：遍历 ELF 的 `ELF_PROG_LOAD` 段，为每个段创建 VMA，记录 `filesz` 用于区分文件数据和 `bss` 零填区。
...
if (ph.flags & ELF_PROG_FLAG_READ)  newvma[vma_idx].prot |= PROT_READ;
if (ph.flags & ELF_PROG_FLAG_WRITE) newvma[vma_idx].prot |= PROT_WRITE;
if (ph.flags & ELF_PROG_FLAG_EXEC)  newvma[vma_idx].prot |= PROT_EXEC;
...
- **缺页加载复用 mmap_handler**：命中 VMA 后分配物理页并按需读取文件，`page_off >= filesz` 的部分保持零页（对应 `bss`）。
...
- **系统调用路径补偿缺页**：`copyin/copyout/copyinstr` 在 `walkaddr` 失败时调用 `ensure_user_page`，保证按需段在系统调用访问时也能正确补页。
...
- `/user/program/bigbss`：构造超大 `bss` 并跨页写入，验证零填充与可写性。
...
- `/user/test/demandloadtest`：通过 `exec("bigbss")` 验证代码段、数据段、`bss` 的按需加载路径。
...
- 已纳入 `usertests` 回归，确保与 swap/COW/懒分配共存时仍能稳定通过。

**测试结果：**

<img src="../img/demandloadtest.png" alt="demandloadtest 结果" width="300" />
...
| 特性 | 章节 | 关键改动 | 关键数据结构 | 对应测试 |
|---|---|---|---|---|
| 写时复制（COW） | 2.1 | RO 共享 + 写时复制 | PTE_F、引用计数 | cowtest |
| 懒分配 | 2.2 | 缺页时分配 | 页表、缺页处理 | lazytest |
| mmap | 2.3 | VMA + 文件映射 | VMA 数组 | mmaptest |
| Swap | 2.5 | Clock + swap 槽 | swapmap、PTE_S | swaptest |
| mmap 页换出 | 2.5 | MAP_SHARED 写回 | VMA + 文件回写 | mmapswaptest |
| 动态装入 | 2.6 | exec 建 VMA | VMA + ELF 段 | demandloadtest |

#### 3.7.2 测试覆盖

| 测试用例 | 覆盖点 | 通过标准 | 备注 |
|---|---|---|---|
| swaptest | swap in/out | 校验数据一致性 | 压力测试 |
| mmapswaptest | mmap + swap | 文件回写一致 | MAP_SHARED |
| swapconctest | 并发 swap | 无死锁/崩溃 | 并发压力 |
| swapthrashtest | 抖动节流 | 在阈值内完成 | 默认不纳入回归 |
| swapbuftest | swap buffer | 命中率 > 0 | 轻量 PBA |
| demandloadtest | 动态装入 | bigbss 正常执行 | exec 路径 |

#### 3.7.3 设计取舍与局限

- Clock 置换是较轻量的策略，牺牲部分命中率换取可实现性与低开销。
- swap 区域为固定预留，当前未实现在线扩展。
- mmap 页换出仅支持 MAP_SHARED 写回路径，MAP_PRIVATE 仍走匿名页逻辑。
- 按需装入依赖缺页路径，极端情况下可能增大首次访问延迟。
