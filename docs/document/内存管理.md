...
```c
  ...
  `/user/test/swapthrashtest` 多进程反复读写大工作集，制造高频换入/换出场景，验证抖动节流机制不会导致系统崩溃（默认不纳入 `usertests`，避免全量回归过慢）。
  `/user/test/swapbuftest` 在轻度超额内存压力下触发 swapout/swapin，并检查 swap buffer 命中统计不为 0。
  `/user/test/swapbufcmptest` 对照测试：顺序读/逆序读对比 swap buffer 命中与耗时（默认不纳入 `usertests`）。

  `/user/test/swapperftest` 用于性能回归（默认不纳入 `usertests`，避免全量回归过慢），统计多轮全量读写的耗时。
