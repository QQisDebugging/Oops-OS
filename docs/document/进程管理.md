...
- [1 基本功能](#一基本功能)
  ...
  - [1.6 用户程序](#16-用户程序)
- [2 改进与创新](#二改进与创新)
  - [2.1 处理机调度改进](#21-调度改进dynprio--mlfq--llf--中级调度)
    - [2.1.1 多级反馈队列调度（MLFQ/DYNPRIO）](#211-多级反馈队列调度mlfqdynprio)
    - [2.1.2 实时调度（LLF）](#212-实时调度llf)
    - [2.1.3 优先级倒置解决（优先级继承）](#216-优先级倒置与优先级继承pi)
    - [2.1.4 中级调度（进程级挂起/恢复）](#218-中级调度进程级挂起恢复)
  - [2.2 记录型信号量与AND信号量](#22-记录型信号量)
    - [2.2.1 记录型信号量](#221-记录型信号量)
    - [2.2.2 AND型信号量/信号量集](#222-and型信号量信号量集)
  - [2.3 死锁处理](#死锁检测与处理)
    - [2.3.1 死锁检测（等待图）](#226-死锁检测与处理等待图)
    - [2.3.2 死锁避免（银行家算法）](#217-银行家算法死锁避免)
  - [2.4 管程机制](#27-管程机制monitor)
  - [2.5 进程通信](#进程通信)
    - [2.5.1 共享内存IPC](#24-进程通信共享内存)
    - [2.5.2 消息队列MQ](#25-进程通信消息队列)
    - [2.5.3 直接通信IPC](#26-进程通信直接通信)
  - [2.6 内核线程完善](#28-内核线程完善)
  - [2.7 基于中断的定时提醒机制](#23-基于中断的定时提醒机制)
