...
- [一、基本功能](#一基本功能)
  - [1.1 进程控制块](#11-进程控制块)
  - [1.2 处理器管理](#12-处理器管理)
  - [1.3 init进程](#13-init进程)
  - [1.4 proc.c](#14-procc)
  - [1.5 自旋锁](#15-自旋锁)
  - [1.6 用户程序](#16-用户程序)
- [二、改进与创新](#二改进与创新)
  - [2.1 调度改进（DYNPRIO + MLFQ + LLF）](#21-调度改进dynprio--mlfq--llf)
  - [2.2 记录型信号量](#22-记录型信号量)
  - [2.3 基于中断的定时提醒机制](#23-基于中断的定时提醒机制)
  - [2.4 进程通信：共享内存](#24-进程通信共享内存)
  - [2.5 进程通信：消息队列](#25-进程通信消息队列)
  - [2.6 进程通信：直接通信](#26-进程通信直接通信)
  - [2.7 管程机制（Monitor）](#27-管程机制monitor)
  - [2.8 内核线程完善](#28-内核线程完善)
...
OopsOS 进程管理模块在 xv6 基础上扩展了调度策略、同步原语与通信机制，形成完整的进程管理子系统。

<img src="../img/图 1-1：进程管理架构图.png" alt="图 0-1 进程管理模块架构" width="600" />

_图 0-1 进程管理模块架构_

### 进程状态转换

OopsOS 在 xv6 的 5 状态基础上新增 `SUSPENDED` 与 `SUSPENDED_SLEEP`，用于中级调度。

<img src="../img/图 1-2：进程状态转换图.png" alt="图 0-2 进程状态转换图" width="500" />

_图 0-2 进程状态转换图（含中级调度挂起状态）_

### 核心改进一览

| 改进类别 | 功能点 | 关键实现 | 测试程序 |
|---------|--------|---------|----------|
| **调度** | 多级反馈队列（MLFQ） | 多级队列、时间片递增、降级/提升 | `schedtest` |
| **调度** | 动态优先级（DYNPRIO） | 优先级动态调整、抢占式调度 | `currentproc` |
| **调度** | 实时调度（LLF） | 最小松弛度优先、截止期保证 | `llftest` |
| **调度** | 优先级倒置解决 | 优先级继承协议（PI） | `pitest` |
| **调度** | 中级调度 | SUSPENDED/SUSPENDED_SLEEP 状态、进程级挂起/恢复 | `midschedtest` |
| **调度** | 优先级设置 | setPriority 动态调整进程优先级 | `setp` |
| **同步** | 记录型信号量 | sem_create/sem_free/sem_p/sem_v、阻塞队列 | `sh_rw_lock` |
| **同步** | AND型信号量 | sem_p_multi 原子获取多资源 | `semandtest` |
| **同步** | 信号量集 | semset_create/semset_free/semset_p/semset_v/semset_p_multi | `semsettest` |
| **同步** | 管程机制 | mon_create/mon_free/mon_enter/mon_exit、Mesa 语义条件变量 | `monitortest` |
| **同步** | 条件变量 | cond_create/cond_free/cond_wait/cond_signal/cond_broadcast | `monitortest` |
| **死锁** | 死锁检测 | 等待图环路检测、即将阻塞时检测 | `deadlocktest` |
| **死锁** | 死锁避免 | 银行家算法安全性检查（banker_init/set_max/request/release） | `bankertest` |
| **通信** | 共享内存 | shmgetat/shmrefcount、引用计数、零拷贝映射 | `sharemm` |
| **通信** | 消息队列 | mqget/msgsnd/msgrcv、按类型收发、阻塞语义 | `msgtest` |
| **通信** | 直接通信 | dmsgsend/dmsgrcv、PID 定向、点对点通信 | `dmsgtest` |
| **线程** | 用户级线程 | clone/join/gettid/gettgid/thread_exit、共享地址空间、线程组（tgid） | `threadtest` |
| **信号** | 定时器信号 | sigalarm/sigreturn、用户态中断处理 | `alarmtest` |
| **调试** | 进程状态查看 | cps 遍历进程表打印信息 | `currentproc` |
| **调试** | 系统调用跟踪 | trace 设置跟踪掩码 | `trace` |
| **调试** | 系统信息查询 | sysinfo 获取空闲内存/进程数/挂起数 | `sysinfo` |
| **调试** | 页表打印 | print_pgtable 打印当前进程页表 | `print_pgtable` |
| **其他** | 获取父进程PID | getparentpid 返回父进程标识 | `getparentpid` |
| **其他** | 扩展exec | execve 支持环境变量传递 | `execve` |
...
- 进程运行状态：state进程状态，killed是否被杀死
  - state分为{ UNUSED未使用, SLEEPING阻塞, SUSPENDED_SLEEP挂起阻塞, SUSPENDED挂起就绪, RUNNABLE就绪, RUNNING运行中, ZOMBIE僵尸 };
...
**背景：**  
xv6 只有短程调度（RR），没有进程级“挂起/换入”机制；当内存压力增大时只能依赖页级换出。为了对齐课本“中级调度（Swapper）”内容，本项目在页级 swap 之上增加**进程级挂起**：在内存紧张时主动将整个进程换出并从就绪队列中移除，降低多道程度，缓解抖动。

**设计要点：**

1. **新增进程状态**：`SUSPENDED` / `SUSPENDED_SLEEP`，用于区分“已挂起就绪”与“挂起阻塞”。  
2. **触发条件**：在 `kalloc` 的内存压力路径中检测空闲页低于阈值（`MIDSCHED_SUSPEND_MINFREE_PAGES`），选择候选进程进行挂起。  
3. **触发补充**：当 swap buffer 缓存接近上限（`MIDSCHED_SWAPBUF_TRIGGER`）时，视为持续内存压力，也会触发挂起。  
4. **恢复条件**：当空闲页高于阈值（`MIDSCHED_RESUME_MINFREE_PAGES`），且 swap buffer 已回落（`swapbuf_cached < MIDSCHED_SWAPBUF_TRIGGER/2`）时恢复；若空闲页非常充足（`MIDSCHED_RESUME_FORCE_PAGES`），即使 swap buffer 未回落也允许恢复，避免长期挂起。  
5. **最短保持时间**：挂起后至少维持 `MIDSCHED_HOLD_TICKS` 个 tick，避免“刚挂起就立刻恢复”。  
6. **进程级换出**：使用 `swapout_proc()` 对目标进程的用户页进行批量换出，释放物理页。  
7. **开关与回收**：新增 `SYS_midsched`，支持运行期启停；关闭时立即恢复所有挂起进程，避免永久挂起。  
8. **与睡眠/唤醒协同**：被挂起的睡眠进程在 `wakeup` 后从 `SUSPENDED_SLEEP` 变为 `SUSPENDED`，等待恢复。

**关键代码（挂起/恢复）：**

```c
static void midsched_maintain(void)
{
  uint64 free;
  freebytes(&free);
...
{
  // 选择 RUNNABLE/SLEEPING 的大进程作为 victim
  // -> 置为 SUSPENDED / SUSPENDED_SLEEP
  // -> swapout_proc 释放页
}
```
...
void *kalloc(void)
{
  for (;;) {
    ...
    if (r) return r;
    if (swapout() < 0) {
      if (midsched_on() && midsched_suspend_one() > 0)
        continue;
      return 0;
    }
    if (midsched_on()) {
...
- `midschedtest`：
  - 创建大内存进程作为候选 victim；  
  - 通过额外内存压力触发挂起；  
  - 释放压力后观察 `nsuspended` 回落到 0；  
  - `usertests` 中以 `-q` 静默运行。
- `midschedcmp`（手动运行）：  
  - 先关闭中级调度再开启中级调度；  
  - 通过多进程内存压力 + pipe 往返测试对比延迟；  
  - 输出两阶段 elapsed ticks 与 `nsuspended` 差异。

**说明：**

- 中级调度是**进程级**控制多道程度；页级 swap 仍保留，二者协同以降低抖动。
- 为避免打断实时任务，挂起逻辑会跳过 `SCHED_RT_LLF` 进程。
...
<img src="../img/proc_semtest.png" alt="图 2-7 记录型信号量测试" width="500" />

_图 2-7 sh_rw_lock 信号量互斥测试结果_

<img src="../img/proc_semandtest.png" alt="图 2-8 AND型信号量测试" width="500" />

_图 2-8 semandtest AND信号量测试结果_

<img src="../img/proc_semsettest.png" alt="图 2-9 信号量集测试" width="500" />

_图 2-9 semsettest 信号量集测试结果_
...
<img src="../img/proc_deadlocktest.png" alt="图 2-10 死锁检测测试" width="500" />

_图 2-10 deadlocktest 死锁检测测试结果_

<img src="../img/proc_bankertest.png" alt="图 2-11 银行家算法测试" width="500" />

_图 2-11 bankertest 银行家算法测试结果_
...
<img src="../img/proc_sharemm.png" alt="图 2-12 共享内存测试" width="500" />

_图 2-12 sharemm 共享内存测试结果_
...
<img src="../img/proc_msgtest.png" alt="图 2-13 消息队列测试" width="500" />

_图 2-13 msgtest 消息队列测试结果_
...
<img src="../img/proc_dmsgtest.png" alt="图 2-14 直接通信测试" width="500" />

_图 2-14 dmsgtest 直接通信测试结果_
...
<img src="../img/proc_threadtest.png" alt="图 2-16 线程测试" width="500" />

_图 2-16 threadtest 线程测试结果_



## 三、补充与扩写（按统一结构）
以下内容在保留原文的基础上，对“改进与创新”部分按“原理/实现策略/测试程序/优点”补齐结构化描述，并补充流程图与测试截图占位。

### 3.1 调度改进（DYNPRIO + MLFQ）
#### 原理
- **DYNPRIO**：通过等待时间奖励与 CPU 时间惩罚，提升交互响应并缓解饥饿。
- **MLFQ**：短作业优先、长作业下沉，配合周期性提升避免长期饥饿。

#### 实现策略
- `setPriority` 同步更新 `priority/dyn_priority` 与 `mlfq_level`。
- DYNPRIO 在调度器中重新计算 `dyn_priority`，限制范围 `[0,20]`。
- MLFQ 通过 `mlfq_ticks` 统计时间片，耗尽则降级，`mlfq_boost()` 周期性提升。

```c
int base = pi_effective_base(p);
p->dyn_priority = base + (p->wait_time / 5) - (p->cpu_time / 5);
...
<img src="../img/sched_overview.png" alt="图 3-1 调度策略总览" width="400" />

_图 3-1 调度策略总览_

#### 测试程序
- `schedtest`：对比 MLFQ/DYNPRIO 交互延迟与总耗时。
- `setp + cps`：观察优先级与队列层级变化。

测试结果（截图占位）：

<img src="../img/schedtest_result.png" alt="图 3-2 schedtest 对比结果" width="400" />

_图 3-2 schedtest 对比结果_
...
### 3.2 实时调度（LLF）
#### 原理
LLF（Least Laxity First）按“松弛度最小”优先：`laxity = deadline - now - remaining`。适合周期性实时任务，支持抢占。

#### 实现策略
- `rt_set/rt_clear` 配置周期/预算/截止期。
- `rt_pick()` 选择最小 laxity 的 RUNNABLE 任务。
- `rt_tick()` 在时钟中断更新预算与 miss 统计。

```c
int rt_laxity_locked(struct proc *p, uint now)
{
  return (int)p->rt_deadline_tick - (int)now - p->rt_remaining;
}
```

<img src="../img/llf_flow.png" alt="图 3-3 LLF 调度流程" width="400" />

_图 3-3 LLF 调度流程_

#### 测试程序
- `llftest`：normal/rt 模式 miss 对比；非法参数校验。

测试结果（截图占位）：

<img src="../img/llftest_result.png" alt="图 3-4 llftest 对比结果" width="400" />

_图 3-4 llftest 对比结果_
...
### 3.3 中级调度（进程级挂起/恢复）
#### 原理
在内存压力过大时挂起部分进程并 swap out，降低抖动；资源恢复后再唤醒。

#### 实现策略
- `midsched(on)` 控制开关（默认关闭）。
- 选择“可挂起且占用最大的进程”作为 victim。
- 状态区分 `SUSPENDED/SUSPENDED_SLEEP` 便于恢复。
...
<img src="../img/midsched_flow.png" alt="图 3-5 中级调度挂起/恢复流程" width="400" />

_图 3-5 中级调度挂起/恢复流程_

#### 测试程序
- `midschedtest`：触发挂起/恢复并检查 `nsuspended`。
- `midschedcmp`：开关前后交互延迟对比。

测试结果（截图占位）：

<img src="../img/midschedtest_result.png" alt="图 3-6 中级调度测试结果" width="400" />
...
### 3.4 优先级继承（PI）
#### 原理
优先级倒置：高优先级进程阻塞在低优先级锁上。PI 通过“捐赠优先级”提升持锁者，释放后回收。

#### 实现策略
- 进程新增 `pi_boost` 字段。
- `pi_donate()` 更新持锁者 boost；`pi_recalc()` 释放后重新计算。
- 监视器与睡眠锁等待路径触发捐赠。
...
<img src="../img/pi_flow.png" alt="图 3-7 优先级继承流程" width="400" />

_图 3-7 优先级继承流程_

#### 测试程序
- `pitest`：高/中/低优先级竞争，验证倒置缓解。

测试结果（截图占位）：

<img src="../img/pitest_result.png" alt="图 3-8 优先级继承测试结果" width="400" />
...
### 3.5 记录型信号量与信号量集（AND）
#### 原理
记录型信号量支持阻塞同步；AND 语义保证多资源原子获取；信号量集用于批量管理。

#### 实现策略
- `sem_create` 允许初值为 0，`sem_free` 检查等待者。
- `sem_p` 使用信号量锁进行 `sleep`。
- `sem_p_multi` 对 id 排序后原子获取。
- `semset_*` 提供信号量集与 AND。
...
<img src="../img/sem_flow.png" alt="图 3-9 信号量 P/V 与 AND 流程" width="400" />

_图 3-9 信号量 P/V 与 AND 流程_

#### 测试程序
- `semtest`、`semandtest`、`semsettest`（均支持 `-q`）。

测试结果（截图占位）：

<img src="../img/semtest_result.png" alt="图 3-10 信号量测试结果" width="400" />
...
### 3.6 死锁检测（等待图）
#### 原理
基于等待图检测：沿资源持有者链路追踪，形成环即判定死锁。

#### 实现策略
- `owner_of_chan()` 统一解析信号量/信号量集/监视器/睡眠锁持有者。
- `deadlock_detect()` 沿等待链追踪，遇到起点返回失败。
...
<img src="../img/deadlock_flow.png" alt="图 3-11 等待图死锁检测流程" width="400" />

_图 3-11 等待图死锁检测流程_

#### 测试程序
- `deadlocktest`：覆盖信号量/信号量集/管程死锁检测（支持 `-q`）。

测试结果（截图占位）：

<img src="../img/deadlocktest_result.png" alt="图 3-12 死锁检测测试结果" width="400" />
...
### 3.7 银行家算法（死锁避免）
#### 原理
银行家算法通过安全性检查避免不安全分配，仅对显式 banker 接口的逻辑资源生效。

#### 实现策略
- 全局 `banker` 记录总量与可用量。
- `banker_request` 试分配后调用 `banker_is_safe_locked()`，不安全则回滚。
- 进程退出时调用 `banker_release_proc` 归还资源。
...
<img src="../img/banker_flow.png" alt="图 3-13 银行家算法安全性检查" width="400" />

_图 3-13 银行家算法安全性检查_

#### 测试程序
- `bankertest`：安全/不安全请求与回滚路径（支持 `-q`）。

测试结果（截图占位）：

<img src="../img/bankertest_result.png" alt="图 3-14 银行家算法测试结果" width="400" />
...
### 3.8 进程通信：共享内存
#### 原理
共享内存提供零拷贝 IPC，多个进程映射同一物理页实现高吞吐数据交换。

#### 实现策略
- `shmgetat/shmrefcount` 管理映射与引用计数，fork 自动继承。
- 进程退出时自动释放引用并回收。
...
<img src="../img/shm_flow.png" alt="图 3-15 共享内存映射流程" width="400" />

_图 3-15 共享内存映射流程_

#### 测试程序
- `sharemm`、`sh_rw_lock/sh_rw_nolock`。

测试结果（截图占位）：

<img src="../img/sharemm_test.png" alt="图 3-16 共享内存测试结果" width="400" />
...
<img src="../img/msgqueue_flow.png" alt="图 3-17 消息队列通信流程" width="400" />
...
<img src="../img/msgtest_result.png" alt="图 3-18 消息队列测试结果" width="400" />
...
<img src="../img/dmsg_flow.png" alt="图 3-19 直接通信流程" width="400" />
...
<img src="../img/dmsgtest_result.png" alt="图 3-20 直接通信测试结果" width="400" />
...
### 3.11 管程机制（Monitor）
#### 原理
管程将互斥进入与条件变量绑定，采用 Mesa 语义，减少错误使用。

#### 实现策略
- `mon_enter/mon_exit` 互斥进入。
- `cond_wait` 释放监视器并等待，唤醒后重新竞争。
...
<img src="../img/monitor_flow.png" alt="图 3-21 管程与条件变量流程" width="400" />

_图 3-21 管程与条件变量流程_

#### 测试程序
- `monitortest`、`deadlocktest`。

测试结果（截图占位）：

<img src="../img/monitortest_result.png" alt="图 3-22 管程测试结果" width="400" />
...
### 3.12 线程支持（clone/uthread）
#### 原理
线程共享地址空间，创建/回收比进程更轻量；线程组由 `tgid` 统一管理。

#### 实现策略
- `clone` 校验栈对齐并设置 guard page。
- `gettid/gettgid/thread_exit/join` 完整生命周期。
- `uthread` 提供 `thread_create/thread_join` 封装。

```c
int pid = clone((uint64)thread_entry, (uint64)ctx, (uint64)stack_base);
```

<img src="../img/thread_flow.png" alt="图 3-23 线程创建与回收流程" width="400" />
...
<img src="../img/threadtest_result.png" alt="图 3-24 线程测试结果" width="400" />
