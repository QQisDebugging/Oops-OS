...
直接通信以进程 PID 为目标，不需要显式创建信箱/队列。每个进程维护一个消息缓冲队列，队列为空时接收阻塞，队列满时发送阻塞，接收后唤醒等待发送者。

- **系统调用**：`dmsgsend(pid, buf, len)` / `dmsgrcv(buf, len)`
- **语义**：发送阻塞（邮箱满）、接收阻塞（邮箱空）
- **容量限制**：单条消息大小 `DMSG_MAX`，队列长度 `DMSG_QUEUE_MAX`

测试用例：`user/test/dmsgtest.c` 覆盖非法参数、基本收发、小缓冲失败、发送阻塞与多发送者场景。

补充演示：`user/test/cstest.c` 为“客户端-服务器”请求/响应示例。服务端通过 `dmsgrcv` 接收请求并用返回值中的发送方 PID 回包；客户端发送请求并校验回包内容，用于展示直接通信的典型 C/S 用法。
