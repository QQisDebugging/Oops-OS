...
- [1. 总览](#1-总览)
  - [1.1 文件系统总览](#11-文件系统总览)
  - [1.2 核心改进一览](#12-核心改进一览)
  - [1.3 磁盘布局与关键数据结构](#13-磁盘布局与关键数据结构)
  - [1.4 关键流程图](#14-关键流程图)
  - [1.5 核心数据结构速览](#15-核心数据结构速览)
  - [1.6 模块与代码分布](#16-模块与代码分布)
  - [1.7 系统调用覆盖](#17-系统调用覆盖)
- [2. 基本功能（xv6）](#2-基本功能xv6)
  - [2.1 磁盘布局与超级块](#21-磁盘布局与超级块)
  - [2.2 Buffer Cache](#22-buffer-cache)
  - [2.3 日志（WAL）](#23-日志wal)
  - [2.4 inode 与块映射（bmap）](#24-inode-与块映射bmap)
  - [2.5 目录与路径解析](#25-目录与路径解析)
  - [2.6 file 层与文件描述符](#26-file-层与文件描述符)
- [3. 改进与创新](#3-改进与创新)
  - [3.1 二级间接块的混合索引分配方式](#31-二级间接块的混合索引分配方式)
  - [3.2 细粒度化 buffer cache 互斥锁](#32-细粒度化-buffer-cache-互斥锁)
  - [3.3 符号链接](#33-符号链接)
  - [3.4 文件访问控制权限（chmod/access）](#34-文件访问控制权限chmodaccess)
  - [3.5 基于索引信息的文件恢复策略](#35-基于索引信息的文件恢复策略)
  - [3.6 文件预分配（fallocate）](#36-文件预分配fallocate)
  - [3.7 文件截断（truncate/ftruncate）](#37-文件截断truncateftruncate)
  - [3.8 稀疏文件打洞（punch hole）](#38-稀疏文件打洞punch-hole)
  - [3.9 文件克隆与写时复制（fclone）](#39-文件克隆与写时复制fclone)
  - [3.10 文件偏移量随机访问（lseek）](#310-文件偏移量随机访问lseek)
  - [3.11 文件重命名（rename）](#311-文件重命名rename)
  - [3.12 块级在线去重（dedup）](#312-块级在线去重dedup)
  - [3.13 文件范围克隆（fclonerange）](#313-文件范围克隆fclonerange)
  - [3.14 文件锁（flock）](#314-文件锁flock)
  - [3.15 文件同步（fsync/fdatasync）](#315-文件同步fsyncfdatasync)
  - [3.16 文件扩展属性（xattr）](#316-文件扩展属性xattr)
  - [3.17 文件系统空间统计（fsinfo/statfs）](#317-文件系统空间统计fsinfostatfs)
  - [3.18 位置读写（pread/pwrite）](#318-位置读写preadpwrite)
  - [3.19 文件描述符复制（dup2）](#319-文件描述符复制dup2)
  - [3.20 分散/聚集 I/O（readv/writev）](#320-分散聚集-ioreadvwritev)
  - [3.21 虚拟文件系统（VFS）](#321-虚拟文件系统sfs)
  - [3.22 挂载/卸载（mount/umount）](#322-挂载卸载mountumount)
...
| 改进类别 | 功能点 | 关键实现 | 测试程序 |
|---------|--------|---------|----------|
| **大文件** | 二级间接块 | 混合索引分配、扩大 MAXFILE | `bigfiletest` |
| **细粒度锁** | buffer cache 分桶锁 | 哈希桶 + evict_lock | `bcachetest` |
| **软链接** | 符号链接 | T_SYMLINK、路径存储、递归解析 | `symlinktest` |
| **权限** | chmod/access | inode mode 位、读写校验 | `chmodtest` |
| **文件恢复** | geti/recoveri | 索引导出、块地址恢复 | `recoveritest` |
| **预分配** | fallocate | 提前分配块、减少碎片 | `fallocatetest` |
| **截断** | truncate/ftruncate | 回收多余块、更新 inode | `truncatetest` |
| **稀疏文件** | punch hole | 释放区间块、保持 size | `punchtest` |
| **克隆** | fclone | 块级 COW、bref 引用计数 | `fclonetest` |
| **随机访问** | lseek | SEEK_SET/CUR/END | `lseektest` |
| **重命名** | rename | 原子目录项更新 | `renametest` |
| **去重** | dedup | 块内容比较、共享地址 | `deduptest` |
| **范围克隆** | fclonerange | 局部块共享 | `fclonerangetest` |
| **文件锁** | flock | 共享/排他锁、阻塞等待 | `flocktest` |
| **同步** | fsync/fdatasync | 日志提交、数据落盘 | `fsynctest` |
| **扩展属性** | xattr | 键值对元数据 | `xattrtest` |
| **空间统计** | fsinfo | 总块/空闲块/inode 统计 | `fsinfotest` |
| **位置读写** | pread/pwrite | 指定偏移、不更新 f->off | `preadwritetest` |
| **FD复制** | dup2 | 指定编号复制 | `dup2test` |
| **批量I/O** | readv/writev | 多缓冲区一次调用 | `readvwritevtest` |
| **VFS** | 虚拟文件系统 | 统一接口、多 FS 支持 | `vfstest` |
| **挂载** | mount/umount | 路径前缀匹配、运行时扩展 | `vfstest` |
