...
- `offset`：预分配起始偏移（字节）
- `len`：预分配长度（字节）
- `flags`：标志位，支持 `FALLOC_KEEP_SIZE`（0x001）保持原文件大小不变（`FALLOC_FL_PUNCH_HOLE` 见 2.12）
...
- **基本预分配测试**：从 offset=0 预分配 300 个块，验证文件大小正确。
...
实际场景中常需要复制大文件的局部片段（例如日志切片、镜像文件的局部复制）。完整 `fclone` 只能按文件粒度克隆，缺少“局部复制”的高效路径。

`fclonerange` 支持在两个文件之间按块范围共享数据块，减少拷贝时间与磁盘占用，并复用 COW 保证写入隔离。

#### 2.13.2 接口与语义

**系统调用号：** `SYS_fclonerange = 83`

```c
int fclonerange(int srcfd, int srcoff, int dstfd, int dstoff, int len);
```

- `srcoff/dstoff/len` 需按 `BSIZE` 对齐
- `srcoff + len` 不得超过源文件大小
- `dstoff + len` 超出目标文件大小时会扩展目标文件
- 共享数据块，写入触发 COW

#### 2.13.3 实现策略

- 通过 `bmap_nalloc` 获取源块，跳过空洞
- 使用 `bmap_ensure + bmap_set_existing` 在目标文件建立指针并增加 `brefs` 引用计数
- 对目标已有块先释放，保持范围语义一致
- 更新目标文件大小并 `iupdate` 写回

#### 2.13.4 测试

`/user/test/fclonerangetest` 覆盖：

- **范围克隆**：拷贝 3 个块，验证前/后块内容一致
- **空洞复制**：源文件中间空洞克隆后读取为全零
- **COW 隔离**：修改目标块后源文件内容不变

#### 2.13.5 优点

- 支持“部分文件”快速克隆，适配大文件分片场景
- 复用 brefs + COW，无需额外格式扩展
- 与 dedup / fclone 形成完整的块共享体系
