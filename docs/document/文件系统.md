...
- 2.10 文件重命名（rename）
- 2.11 块级在线去重（dedup）
- 2.12 稀疏文件打洞（punch hole）
- 2.13 文件锁（flock）
- 2.14 文件同步（fsync/fdatasync）
- 2.15 文件扩展属性（xattr）
...
对于顺序写入的大文件，提前分配磁盘块可以减少运行时分配与碎片化开销，提高写入性能与数据布局的连续性。这对于数据库日志、视频文件等需要预知大小的场景尤为有用。

#### 2.6.2 设计方案

**系统调用号：** `SYS_fallocate = 74`

**接口：**

```c
int fallocate(int fd, int size, int flags);
```

**参数说明：**
- `fd`：文件描述符，必须以写模式打开
- `size`：预分配的目标大小（字节）
- `flags`：标志位，支持 `FALLOC_KEEP_SIZE`（0x001）保持原文件大小不变
...
uint64 sys_fallocate(void)
{
  ...
  uint b = (f->ip->size + BSIZE - 1) / BSIZE;
...
`/user/test/fallocatetest` 用于验证预分配功能，考虑如下情况：

- **基本预分配测试**：分配 300 个块的空间，验证文件大小正确。
- **零填充验证**：读取预分配区域，确认内容为全零。
- **FALLOC_KEEP_SIZE 测试**：验证分配后文件大小保持不变。

**测试结果：**

所有测试通过，fallocate 能够正确预分配块并支持 KEEP_SIZE 标志。

#### 2.6.4 优点

- 降低大文件写入时的分配与碎片化开销。
- 支持 KEEP_SIZE 标志，可与稀疏文件语义配合。
- 为性能优化和后续改进提供基础支持。
...
通过共享数据块实现近乎 O(1) 的文件复制，写入时采用 COW（Copy-on-Write），避免修改影响原文件。这在镜像文件、快照、复制大文件等场景中非常有用。
...
**语义：**
- 从源文件创建一个克隆文件，目标文件必须不存在
- 克隆文件与源文件共享数据块，不复制实际数据
- 后续对任一文件的写入会触发 COW，保证写入隔离

**实现策略：**

1. **块引用计数（brefs）**：维护每个数据块的引用计数，启动时扫描 inode 重建计数。
2. **克隆时共享块**：`iclone` 复制块指针并增加引用计数。
3. **写时复制**：`bmap` 在写入路径检测到 bref > 1 时执行 COW。
...
`/user/test/fclonetest` 用于验证克隆功能，考虑如下情况：

- **克隆存在文件失败测试**：目标文件已存在时返回错误。
- **内容一致性测试**：克隆后读取目标文件，验证与源文件内容一致。
- **COW 隔离测试**：修改克隆文件后，验证源文件内容未变化。
- **克隆文件更新测试**：验证克隆文件写入后数据正确更新。

**测试结果：**

所有测试通过，fclone 能够正确共享数据块并保证 COW 隔离。

#### 2.7.4 优点

- 节省复制时间与磁盘空间，实现 O(1) 级别的文件克隆。
- 提供类 reflink 的创新能力，适用于镜像、快照等场景。
- 与块引用计数自然协作，保证写入隔离。
...
对 `dstpath` 执行去重，逐块比较 `srcpath` 与 `dstpath` 的重叠范围（最多到 `min(src.size, dst.size)`）。当双方对应块都已分配且内容完全一致时，将 `dst` 的块指针改为指向 `src` 的块、增加 `src` 块引用计数，并释放 `dst` 原块。去重后对共享块的写入会触发 COW。

**返回值：** 成功返回“新共享”的数据块数量；失败返回 `-1`。

#### 2.11.3 测试

`/user/test/deduptest` 覆盖如下场景：

- **块共享测试**：构造两个部分相同的文件（覆盖 direct + single-indirect 块），调用 `dedup` 后使用 `geti + recoveri` 校验 blockno 级别的共享关系。
- **COW 隔离测试**：修改 `dst` 的共享块后，验证 `src` 内容未变化。
- **边界条件**：验证相同文件、不同设备等边界情况的处理。
...
**测试结果：** 所有测试通过，能正确共享相同块并保证 COW 隔离。
...
在日志文件、数据库、虚拟磁盘镜像等场景中，文件中可能存在大量不再需要的数据区域。传统的 `truncate` 只能截断文件末尾，无法释放文件中间的块。

稀疏文件打洞（punch hole）功能允许释放文件中任意范围的数据块，读取该区域时返回全零，文件大小保持不变。这是 ext4、XFS、Btrfs 等现代文件系统的标准特性。
...
// 新增标志：
#define FALLOC_FL_PUNCH_HOLE 0x002  // 释放 [offset, offset+len) 范围的块
```

**语义：**
- 当 `flags` 包含 `FALLOC_FL_PUNCH_HOLE` 时，释放指定范围内的数据块
- 文件大小保持不变
- 读取被释放的区域返回全零
- 后续写入该区域会分配新块
...
为正确支持稀疏文件，修改了 `readi` 函数：当读取块指针为 0 的区域时，直接返回全零而不尝试从磁盘读取，避免在事务外调用 `bmap` 分配新块。
...
int readi(struct inode *ip, int user_dst, uint64 dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    blkaddr = bmap_nalloc(ip, off/BSIZE);  // 不分配，只查询
    if(blkaddr == 0){
      // 稀疏空洞：直接返回零，不读磁盘
...
`/user/test/punchtest` 覆盖如下场景：

- **基本打洞测试**：创建 5 个块的文件，打洞中间 3 个块，验证前后块不变、打洞区域读取为零。
- **文件大小不变测试**：打洞后文件大小保持不变。
- **写入打洞区域测试**：向打洞区域写入数据，验证新块分配正确。
- **其他打洞块独立性测试**：写入一个打洞块不影响其他打洞块。

**测试结果：** 所有测试通过，punch hole 能正确释放文件中间的块。

#### 2.12.4 优点

- 支持释放文件中间区域的磁盘空间，不仅限于截断末尾。
- 与 fallocate 预分配形成完整的空间管理体系。
- 与块引用计数自然协作，共享块只减少计数不影响其他文件。
- 适用于日志回收、数据库空间重用、虚拟磁盘稀疏化等场景。
- 符合现代文件系统标准（ext4/XFS/Btrfs 均支持）。

### 2.13 文件锁（flock）

文件锁（flock）提供进程间文件访问协调机制，支持共享锁（读锁）和排他锁（写锁），允许多进程安全地并发访问同一文件。
...
- `fd`：已打开的文件描述符
- `operation`：锁操作标志
  - `LOCK_SH`（1）：共享锁（读锁），允许多个进程同时持有
  - `LOCK_EX`（2）：排他锁（写锁），只允许一个进程持有
  - `LOCK_UN`（8）：解锁
  - `LOCK_NB`（4）：非阻塞模式，可与 `LOCK_SH`/`LOCK_EX` 组合

**返回值**：
- 成功返回 0
- 失败返回 -1（非阻塞模式下无法立即获取锁时返回）
...
`/user/test/flocktest` 覆盖如下场景：

1. **共享锁测试**：验证多个进程可同时持有共享锁
2. **排他锁测试**：验证排他锁互斥，非阻塞模式正确返回失败
3. **锁升级测试**：验证释放共享锁后可获取排他锁
4. **阻塞等待测试**：验证进程在锁被释放后被唤醒
5. **共享/排他冲突测试**：验证共享锁阻止排他锁，但允许其他共享锁

#### 2.13.4 优点

- **读写并发**：共享锁允许多读者并发，提高读密集型场景性能
- **写入互斥**：排他锁保证写入操作的原子性和一致性
- **非阻塞支持**：`LOCK_NB` 允许应用程序实现轮询或超时逻辑
- **简单易用**：与 POSIX flock() 语义一致，便于移植
- **基于 inode**：锁与文件关联，不同 fd 打开同一文件共享锁状态

### 2.14 文件同步（fsync/fdatasync）

文件同步系统调用确保文件数据和/或元数据被持久化到磁盘，是数据库、日志系统等需要数据持久性保证的应用的基础设施。

#### 2.14.1 接口设计

**系统调用**：
```c
int fsync(int fd);      // 同步数据 + 元数据
...
| 同步文件元数据（inode） | ✅ | ❌ |
| 性能 | 较慢 | 较快 |
| 适用场景 | 需要完整持久性 | 仅关心数据内容 |

**实现代码**：

```c
// file.c 中的 filefsync() 实现
...
xv6 使用日志（logging）保证崩溃一致性：
1. 文件写操作通过 `log_write()` 记录到日志
2. `end_op()` 触发事务提交（commit）
3. `fsync()` 强制执行一次完整的事务提交流程

```
write() → log_write() → [缓存]
                            ↓
fsync() → end_op() → commit() → 磁盘
```

#### 2.14.3 测试

`/user/test/fsynctest` 覆盖如下场景：

1. **fsync 基本功能**：写入数据后 fsync，验证数据持久化
2. **fdatasync 基本功能**：仅同步数据，跳过元数据更新
3. **多次写入后 fsync**：多次写入后一次性同步
4. **无效文件描述符**：对无效 fd 调用应返回错误

#### 2.14.4 优点

- **数据持久性保证**：确保关键数据不因系统崩溃而丢失
- **性能优化选项**：fdatasync 跳过元数据更新，减少 I/O
- **与日志系统集成**：利用现有日志机制实现原子性
- **POSIX 兼容**：与标准 Unix 系统调用语义一致
- **简单易用**：单参数接口，易于集成到应用程序

### 2.15 文件扩展属性（xattr）

文件扩展属性（Extended Attributes）允许为文件附加任意的 key-value 元数据，是现代文件系统的重要特性。这些属性独立于文件内容存储，可用于存储标签、安全上下文、自定义元数据等。
...
| 系统调用 | 参数说明 |
|----------|----------|
| `setxattr` | path: 文件路径, name: 属性名, value: 属性值, size: 值长度 |
| `getxattr` | path: 文件路径, name: 属性名, value: 接收缓冲区, size: 缓冲区大小 |
| `listxattr` | path: 文件路径, list: 接收缓冲区, size: 缓冲区大小（传 0 仅查询长度） |
| `removexattr` | path: 文件路径, name: 要删除的属性名 |

**返回值**：
- `setxattr`：成功返回 0，失败返回 -1
- `getxattr`：成功返回值长度，失败返回 -1
- `listxattr`：成功返回列表总长度，失败返回 -1
- `removexattr`：成功返回 0，失败返回 -1

**使用示例**：
```c
// 设置属性
setxattr("/data/file.txt", "user.author", "alice", 5);
...
// 删除属性
removexattr("/data/file.txt", "user.tag");
```
...
采用内存中的 xattr 表存储，以 (dev, inum) 为键：
...
```c
// 设置属性
int xattr_set(struct inode *ip, char *name, char *value, int size)
{
  ...
  for(int i = 0; i < XATTR_MAX_ATTRS; i++) {
    if(xi->attrs[i].used && strcmp(xi->attrs[i].name, name) == 0) {
      // 更新现有属性
      memmove(xi->attrs[i].value, value, size);
...
`/user/test/xattrtest` 覆盖如下场景：

1. **设置和获取属性**：基本的 set/get 功能
2. **多个属性**：同一文件设置多个属性
3. **列出属性**：listxattr 返回所有属性名
4. **删除属性**：removexattr 删除单个属性
5. **更新属性**：重新设置同名属性覆盖旧值
6. **不存在的属性**：获取不存在的属性返回错误

#### 2.15.4 优点

- **灵活扩展**：无需修改文件内容即可附加元数据
- **命名空间**：支持 `user.xxx` 等命名约定
- **应用广泛**：可用于标签、ACL、安全上下文、版本信息等
- **简单接口**：与 Linux xattr 接口兼容
- **独立存储**：属性与文件数据分离，不影响文件内容
